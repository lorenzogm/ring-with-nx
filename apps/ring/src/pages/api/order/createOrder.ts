...
const createOrder: NextApiHandler = async (req, res) => {
  ...
  if (!config.emails.orderConfirmationTemplateId) {
    res.status(500).json({
      message: `Undefined "Order confirmation template ID". Please get the template ID from Sendinblue and add it to the "Config" document in Prismic.`,
    })
